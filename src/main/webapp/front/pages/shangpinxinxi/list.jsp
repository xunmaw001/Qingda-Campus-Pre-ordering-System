        ...
        <fieldset class="search-container" style="text-align: center;margin: 30px 0;">
          <form class="layui-form">
            ...
            <div class="layui-inline" style="margin-bottom: 10px;">
              ...
              <div class="layui-input-inline">
                <input type="text" name="shangpinfenlei" id="shangpinfenlei" placeholder="商品分类" autocomplete="off" class="layui-input">
              </div>
            </div>   
            <%--<div class="layui-inline" style="margin-bottom: 10px;">--%>
            ...
            <div class="layui-inline" style="margin-left: 30px;margin-bottom: 10px;">
              ...
              <button v-if="isAuth('shangpinxinxi','新增')" @click="jump('../shangpinxinxi/add.jsp')" type="button" class="layui-btn btn-theme">
          ...
          <ul class="after">
            <li v-for="(item,index) in dataList" v-bind:key="index">
              <a target="_self" :href="'../shangpinxinxi/detail.jsp?id='+item.id">
                <div class="posi-img">
                  <img :src="item.fengmian?item.fengmian.split(',')[0]:''" width="385px" height="220px" />
                </div>
                <div class="remark">
                  <h5>商品:{{item.shangpinmingcheng}}</h5>
                  <span style="color: red" size="30">¥{{item.discountprice}}</span>   原价: <s>{{item.price}}</s>
                </div>
              </a>
            </li>
          </ul>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
    	...
    	if (url.indexOf("?") != -1) {  
  	      ...
  	      strs = str.split("&");  
  	      for(var i = 0; i < strs.length; i ++) {  
  	    	param['shangpinmingcheng']='%'+unescape(strs[i].split("=")[1])+'%';
  	      }  
  	   }  
        ...
        if (jquery('#guige').val()) {
          param['guige'] = jquery('#guige').val() ? '%' + jquery('#guige').val() + '%' : '';
        }  
        // 获取列表数据
